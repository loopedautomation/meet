  ...
  try {
    await roomService().updateParticipant(slug, identity, {
      ...
      permission: {
        ...
        canPublishData: true,
        canPublishSources: [],
        // Admitted participants can raise their hand / set attributes,
        // matching the grant fresh joiners get in the token route.
    ...
    // Matching the deny path: an unhandled throw here escaped as a 500 and
    // told the host nothing about why the knocker never got in.
    return NextResponse.json({ error: "admit failed" }, { status: 502 })
  }
  return NextResponse.json({ ok: true })
}
